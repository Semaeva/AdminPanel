        ...
        public async Task<IActionResult> Register(RegisterViewModel model) {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await emailServices.SendEmailAsync(model.Email, "Подтверждение Вашего аккаунта",
                        $"Подтсердите Вашу регистрацию, перейдся по ссылке: " +$" <a href='{callbcak}'>link<a> ");
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if(user != null)
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, "Вы не подтвердили регистарцию");
                        return View(model);
                    }
                }
